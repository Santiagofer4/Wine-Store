...
function ProductForm() {
  ...
  function handleOnSubmit (e) {
    e.preventDefault()
    if (state.cepa && state.marca && state.price && state.description && state.yearHarvest && state.image && state.stock) {
  ...
  return (
    <div className='formProd'>
      <form onSubmit={(e) => {handleOnSubmit(e)}} className="SearchBar__form" noValidate autoComplete="off">
        ...
        <TextField id={state.marca ? 'accepted' : 'error'}       name='marca'       label="Marca"       type='string'  onChange={(e) => {handleOnChange(e)}}/>
        <TextField id={state.cepa ? 'accepted' : 'error'}        name='cepa'        label="Cepa"        type='string'  onChange={(e) => {handleOnChange(e)}}/>
